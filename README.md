...
- **TTF + 字符集** — 与 GB2312/GBK/Big5 字符集计算差集，按 500 字一组，勾选组别后逐组生成。差集结果可导出为分组 TXT 文件
- **私有区汉字检测** — 自动识别文本中的 PUA 私用区字符（U+E000–U+F8FF），扫描系统字体字形名，尝试还原为标准 Unicode 并给出映射建议
- **失败重试** — 粘贴迭代失败的字符重试
...
**字符集差集计算：** 上传 TTF 字体后，与 GB2312（6763字）/ GBK（20902字）/ Big5（繁体）字符集求差集，自动分组为每 500 字一组，支持勾选组别后逐组生成。差集结果可导出为分组 TXT 文件，方便按批次管理生字任务。

**私有区汉字（PUA）检测：** 部分旧字体使用 Unicode 私用区（U+E000–U+F8FF）编码汉字，导致文本复制后显示为乱码或空白。本工具自动检测文本中的 PUA 字符，扫描 Windows 系统字体（宋体、楷体、仿宋、微软雅黑、明体）的 cmap 表和字形名，尝试将 PUA 编码还原为标准 Unicode，并给出映射建议。检测结果包含：PUA 编码、当前显示、系统字体是否收录、建议的标准映射。
